...
const levels = ['10', '10+', '11', '11+', '12', '12+', '13', '13+', '14', '14+', '15', '15+', '16'];

const DifficultySummary = ({ data , isExport = false}: DifficultySummaryProps) => {
			...
			<div className = {Style}>
				{levels.map((lv) => {
					...
					const getRate = (count: number) =>
						stats.master.total + stats.ultima.total > 0 ? Math.round((count / (stats.master.total + stats.ultima.total)) * 100) : 0;
								...
								<div className="space-y-2 pt-2 border-t border-slate-50">
									<div className="flex justify-between text-[10px] font-black">
										<div className="text-center">
											...
											<p className="text-yellow-500">{stats.master.ajc + stats.ultima.ajc}</p>
										</div>
										<div className="text-center border-x border-slate-50 px-2">
											<p className="text-slate-300 text-[8px] leading-tight">AJ</p>
											<p className="text-orange-500">{stats.master.aj + stats.ultima.aj}</p>
										</div>
										<div className="text-center">
											<p className="text-slate-300 text-[8px] leading-tight">SSS</p>
											<p className="text-rose-400">{stats.master.sss + stats.ultima.sss}</p>
										</div>
									</div>
									...
									<div className="relative h-1 w-full bg-slate-50 rounded-full overflow-hidden">
										<div
											...
											style={{ width: `${getRate(stats.master.sss + stats.ultima.sss)}%` }}
										></div>
										<div
